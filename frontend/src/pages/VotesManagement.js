    ...
    <div className="page" style={{ textAlign: 'center' }}>
      ...
      <ul>
        {categories.map((category) => (
          <li key={category.name}>
            ...
            <button onClick={() => handleDeleteCategory(category.name)}>Supprimer</button>
          </li>
        ))}
      </ul>
      ...
      {editingVote && (
        <div style={{ marginTop: '20px' }}>
          ...
          <br />
          <button onClick={handleUpdateVote}>Mettre à jour</button>
          <button onClick={() => setEditingVote(null)}>Annuler</button>
        </div>
      )}
    </div>
  );
