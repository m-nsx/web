...
function Vote() {
  ...
  return (
    <div className="page">
      ...
      <br />
      {!isPlaying ? (
        <button className="vote-button" onClick={startGame}>Commencer le mini-jeu</button>
      ) : (
        <div
          style={{
            position: 'relative',
      ...
      <br />
      {!isPlaying && score > 0 && (
        <button className="vote-button" onClick={handleVote}>Soumettre le vote avec un score de {score}</button>
      )}
      {message && <p>{message}</p>}
      <br />
